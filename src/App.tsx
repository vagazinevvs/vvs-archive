...
// Handle both local pipeline assets and external proxies
const formatImageUrl = (url: string): string => {
  if (!url) return '';

  // Resolve relative paths generated by pipeline (e.g., ./cards/ID.webp)
  if (url.startsWith('./') || url.startsWith('cards/')) {
    const cleanPath = url.replace(/^\.\/?/, '');
    return `${import.meta.env.BASE_URL}${cleanPath}`;
  }

  // Fallback for direct Google Drive URLs
  ...
  // Proxy external Twitter/X images to prevent CORS issues
...
export default function App() {
  ...
  // Fetch static JSON generated by GitHub Actions pipeline
  useEffect(() => {
    let isMounted = true;
    setLoading(true);
    ...
    const loadCards = async () => {
      try {
        ...
        if (isMounted) {
          const validCards = data.filter((c) => c.id && c.name);
          setCards(validCards);
        }
      } catch (err) {
  ...
  // Sync collected cards to localStorage
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 pb-16">
      ...
      <main className="max-w-7xl mx-auto px-6 pt-6">
        ...
        {/* Card Grid Content State */}
        {loading ? (
          <div className="flex flex-col items-center justify-center py-24 text-slate-400">
            <Loader2 className="h-8 w-8 animate-spin text-indigo-600 mb-2" />
            <p className="text-sm font-medium">Loading cards from archive...</p>
          </div>
        ) : (
          <div
            ref={templateRef}
